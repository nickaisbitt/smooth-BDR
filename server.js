...
import LeadScoringHistory from './services/leadScoringHistory.js';
import CampaignROI from './services/campaignROI.js';
import CompetitorIntelligence from './services/competitorIntelligence.js';
...
// ============ ADVANCED BUSINESS INTELLIGENCE (#61-64) ============

// GET /api/analytics/lead-score-trend/:leadId - Lead scoring trends
app.get('/api/analytics/lead-score-trend/:leadId', async (req, res) => {
    try {
        const trend = await LeadScoringHistory.getScoreTrend(db, req.params.leadId, req.query.days || 30);
        res.json(trend);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// GET /api/analytics/campaign-roi/:campaignId - Campaign ROI analysis
app.get('/api/analytics/campaign-roi/:campaignId', async (req, res) => {
    try {
        const roi = await CampaignROI.calculateROI(db, req.params.campaignId);
        res.json(roi);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// GET /api/analytics/efficiency-metrics - Overall efficiency metrics
app.get('/api/analytics/efficiency-metrics', async (req, res) => {
    try {
        const metrics = await CampaignROI.getEfficiencyMetrics(db);
        res.json(metrics);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// GET /api/analytics/competitive-landscape/:leadId - Competitor intelligence
app.get('/api/analytics/competitive-landscape/:leadId', async (req, res) => {
    try {
        const landscape = await CompetitorIntelligence.getCompetitiveLandscape(db, req.params.leadId);
        res.json(landscape);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
